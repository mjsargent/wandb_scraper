...
inset_location = [14000, 0.05 ,6000, 0.25]
...
for a, env in enumerate(reversed(envs)):
    ...
    plt.ylabel("Average Action Policy Variation", fontsize="x-large")
    plt.xlim(8000,20000)
    plt.ylim(-0.1,0.5)
    ...
    plt.grid()
    plt.savefig(f"./figures/Avg_Total_Var_{env}_skip_{num_skip}.png", dpi=1200)
    if a == 2:
